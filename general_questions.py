...
# Import your continuous learning function
from continuous_training import continuous_training

# Create the blueprint
general_questions = Blueprint('general_questions', __name__)
...
@general_questions.route('/upload', methods=['POST'])
@cross_origin()
def upload_video():
    ...
    # Trigger continuous learning if threshold is met
    THRESHOLD = 10
    csv_files = glob.glob(os.path.join(output_folder, "*.csv"))
    print(f"Found {len(csv_files)} CSV files in '{output_folder}'.")
    if len(csv_files) >= THRESHOLD:
        print("Threshold reached. Triggering continuous learning process...")
        thread = threading.Thread(target=continuous_training(), args=(output_folder,))
        thread.start()
    else:
